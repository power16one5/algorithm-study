...
class SegmentTree():
    ...
    def lazy_propagation(self, node):
    ...
    def update_range(self, s, e, v):
        ...
        self._update_range(1, 0, self.adj - 1)

    def _update_range(self, node, node_s, node_e):
        if node_s >= self.s and node_e <= self.e: 
            self.lazy[node] += self.v
            self.lazy_propagation(node)
            return

        if self.lazy[node]: self.lazy_propagation(node)
        if node_e < self.s or node_s > self.e: return
        
        next_node1 = node << 1
        next_node2 = next_node1 + 1
        mid = (node_s + node_e) >> 1
        self._update_range(next_node1, node_s, mid)
        self._update_range(next_node2, mid + 1, node_e)
